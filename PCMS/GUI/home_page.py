...
class HomePage(ctk.CTkFrame):
    def __init__(self, parent, controller, jobber_service: JobberService, invoice_service: InvoiceService):
        ...
        button3 = ctk.CTkButton(self, text="Generate Invoice",command=self.start_generating_invoice)
        button3.pack(pady=10)

        self.generating_invoice_label = ctk.CTkLabel(self, text="", font=("Helvetica", 12))
        self.generating_invoice_label.pack(pady=5)

    def start_generating_invoice(self):
        self.generating_invoice_label.configure(text="Generating Invoice...")
        try:
            job_data_generated = self.generate_invoice()
            if job_data_generated is None:
                self.generating_invoice_label.configure(text="No Jobs to be processed")
            else:
                self.generating_invoice_label.configure(text="Invoice Generated Successfully")
        except Exception as e:
            logger.error(f"Error: {str(e)}")
            self.generating_invoice_label.configure(text="Error", text_color="red")
